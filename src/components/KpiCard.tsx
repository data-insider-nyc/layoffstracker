...
const KpiCard: React.FC<KpiCardProps> = ({ title, value, note = "" }) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg p-4 text-center border dark:border-gray-700 transition-colors duration-200">
      <h3 className="text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400 leading-tight">{title}</h3>
      <p className="text-xl sm:text-2xl font-medium text-gray-900 dark:text-white mt-1">{formattedValue}</p>
